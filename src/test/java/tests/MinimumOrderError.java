...
public class MinimumOrderError {
	...
	private static final Logger logger = LogManager.getLogger(MinimumOrderError.class);
	...
	public void beforeClass() throws ParserConfigurationException, SAXException, IOException {
		...
		restaurantSearch = new Restaurant(driver);
	 ...
	 * 		Given: Client is restaurant page 
	 * 		When: click on dish with a price below the minimum
	 *  	Then: Receive an error message on the screen that the price is below the minimum
	 */

	@Test(priority = 1, enabled = true, description = "Verify an error message that the price is below the minimum")
	public void MininumOrderTest() throws InterruptedException, IOException, ParserConfigurationException, SAXException {

		logger.info("Going into click on minimum order dish");
		main.login();
		login.doLoginFacebook(userName, password, name);
		Assert.assertTrue(restaurantSearch.mininumOrder(restaurantName));

		logger.info("successfully received an error message");
