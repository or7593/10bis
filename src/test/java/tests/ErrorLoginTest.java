...
public class ErrorLoginTest {
	...
	private static final Logger logger = LogManager.getLogger(ErrorLoginTest.class);
	 ...
	 * 		Given: Client is in login page 
	 * 		When: click on login link Without filling in details 
	 *  	Then: Getting error message 
	 */
	
	@Test(priority = 1, enabled = true, description = "verify eror message on page")
	public void errMessage() throws InterruptedException, IOException {

		logger.info("Going to connection page");
		main.login();
		logger.info("try to login without type email");
		Assert.assertTrue(login.errorMessage(), "Failed verify the Error message");
